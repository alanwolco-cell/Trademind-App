...
async function research(name, team, pos) {
  ...
  let txt = '';
  const diag = [];
  // web search pauses the turn; resume until the model actually answers
  for (let i = 0; i < 4; i++) {
...
router.get('/', async (req, res) => {
  try {
    ...
    let doc = await readBlob(key);
    if (!fresh(doc)) {
      if (inflight[key]) { doc = await inflight[key]; }
      else {
        inflight[key] = research(name, String(req.query.team || '').slice(0, 4), String(req.query.pos || '').slice(0, 4))
          .finally(() => { delete inflight[key]; });
        doc = await inflight[key];
        await put('scout2/' + key + '.json', JSON.stringify(doc), { access: 'public', addRandomSuffix: false, allowOverwrite: true, cacheControlMaxAge: 0 });
      }
    }
    mem[key] = { doc };
