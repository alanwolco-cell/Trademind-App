...
router.get('/adp', async (req, res) => {
  try {
    ...
    const players = {}; const byId = {}; const q6 = {}; const qbTds = []; let source = 'sleeper'; let consensusN = 0;
    ...
    for (let attempt = 0; attempt < 3 && !ok; attempt++) {
      try {
        ...
        (rows || []).forEach(row => {
          ...
          if (!name) return;
          players[norm(name)] = { adp, pos: pl.position || '', team: pl.team || '', name, _id: String(row.player_id) };
